...
import { getNoticeDetail } from '_r/home'

const route = useRoute()

let loading = ref(false)
let noticeDetail = ref({
	title: '',
	content: '',
	createTime: '',
	modifyTime: ''
...
const noticeId = computed(() => {
	return route.params.noticeId
})
watch(noticeId, (newValue) => {
	if (newValue) {
		getNoticeDetailInfo()
	}
})
 ...
 * 获取公告详情
 */
const getNoticeDetailInfo = () => {
	loading.value = true
	getNoticeDetail({ noticeId: noticeId.value }).then((res) => {
		loading.value = false
		if (res.success) {
